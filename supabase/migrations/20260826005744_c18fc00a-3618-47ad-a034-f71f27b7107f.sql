...
begin
  ...
  if ra.id is null then
    v := v || 'riot_account_missing'::text;
  else
    if ra.solo_tier is null or not ra.data_verified then v := v || 'riot_rank_unverified'::text; end if;
    if coalesce(ra.account_level, 0) < v_min then v := v || 'account_level_below_minimum'::text; end if;
    if t.required_platform is not null and lower(coalesce(ra.platform,'')) <> lower(t.required_platform) then
      v := v || 'platform_mismatch'::text;
    end if;
