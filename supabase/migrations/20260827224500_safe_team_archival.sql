...
create index if not exists teams_archived_by_profile_id_idx
  on public.teams(archived_by_profile_id);
create index if not exists teams_archived_at_idx
  on public.teams(archived_at);

drop index if exists public.teams_name_lower_unique;
drop index if exists public.teams_tag_lower_unique;
create unique index teams_name_lower_active_unique
  on public.teams(lower(name)) where archived_at is null;
create unique index teams_tag_lower_active_unique
  on public.teams(lower(tag)) where archived_at is null;
...
begin
  ...
  if exists(select 1 from public.teams where archived_at is null and lower(name) = lower(v_name)) then
    raise exception 'team_name_taken';
  end if;
  if exists(select 1 from public.teams where archived_at is null and lower(tag) = lower(v_tag)) then
    raise exception 'team_tag_taken';
  end if;

  select id, division_id, city_id, country_id, region_id
    into p
...
begin
  ...
  update public.teams
    set archived_at = now(), archived_by_profile_id = v_profile
    where id = v_team;
...
revoke all on function public.archive_my_team() from public;
revoke all on function public.archive_my_team() from anon;
grant execute on function public.archive_my_team() to authenticated;
